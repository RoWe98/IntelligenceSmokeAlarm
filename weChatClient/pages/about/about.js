...
const options = {
  clientId: "zhangjihhshshkkkhhhehellozj",
...
Page({
  ...
  msgback: function (topic, message) {
    ...
    this.onReady(1, (json1["reported"].iii - 200) / 10);
  ...
  onReady: function (start, end) {
    ...
    cxt_arc.arc(106, 106, 95, 0, 2 * Math.PI, false);//设置一个原点(106,106)，半径为100的圆的路径到当前路径
    ...
    cxt_arc.setLineCap('round')
    cxt_arc.beginPath();//开始一个新的路径
    cxt_arc.arc(106, 106, 95, -Math.PI * start / 2, Math.PI * end / 5, false);
    cxt_arc.stroke();//对当前路径进行描边
